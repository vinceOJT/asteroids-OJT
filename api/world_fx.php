...
$db = new MySQLi("localhost", "tetris_app","Xb0xFriyay!", "pacman");
...
// CREATE TABLE `pacman`.`system_config` ( `sc_id` INT NOT NULL AUTO_INCREMENT , `sc_on` TINYINT(1) NOT NULL ,
//  `sc_description` TEXT NOT NULL , `x` TINYINT(1) NOT NULL , PRIMARY KEY (`sc_id`)) ENGINE = InnoDB;
...
function update_score($data) {
    ...
    // $server_ip = gethostbyname($hostname);
            ...
            // echo '<script>console.log("' . $data['ip'] . '");</script>';
            // echo '<script>console.log("' . $data['score'] . '");</script>';
            // echo '<script>console.log("' . $_SERVER['REMOTE_ADDR'] . '");</script>';
            // echo "Your IP address is: " . $server_ip;
        
// After this code runs, $_POST['ip'] will be "192.168.15.2"

    // if (is_null($data['ip'])) {
    //         echo 'The value is NULL.';
    // } else {
    //     echo 'The value is not NULL (or is not defined).';
    // }
  ...
  // $sql = "SELECT s_id, s_score FROM scores WHERE 1 AND s_ip = INET_ATON(\"{$data['ip']}\")";
    // $res = $db->query($sql)->fetch_assoc();

    // if (!$res['s_id']) {
    //     $sql = "INSERT INTO scores (s_ip, s_name, s_data, s_score) VALUES (INET_ATON('{$data['ip']}'), '{$data['name']}', '" . json_encode($data) . "', '{$data['score']}')";
    //     $db->query($sql);    
    // } else {

    
        // $sql = "SELECT s_id, s_score FROM scores WHERE 1 AND s_ip = \"{$data['ip']}\"";
        $sql = "SELECT s_id, s_score FROM scores WHERE 1 AND s_name = \"{$data['name']}\"";
